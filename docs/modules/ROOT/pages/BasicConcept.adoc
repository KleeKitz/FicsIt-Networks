...
A Network Connector is simply a point of a building were you can connect a network cable to and so, connect multiple different network components with each other.
...
Network Components can have nick, which is a list of custom names separated by spaces allowing you to group and name your network components.
...
filter4 = "Power Nice" -- nick filter matches no one
filter5 = "" -- nick filter matches every component (also components with no nick)
----

=== Lua

Every network component can be represented in lua as a table, the Lua-Component-Structure.
...
Also, quite interesting is, a function can have multiple output values which all will exist.
...
Signals are quite important because they allow us to listen for events happening in a network. That means we get notified when something happens and don't need to ask if there happened anything every tick.
...
If a computer wants to receive the signals of an object, he needs to subscribe to the instance (listen to the instance). A computer can just subscribe once at a time to the same instance, but it can subscribe to multiple different ones at the same time.
